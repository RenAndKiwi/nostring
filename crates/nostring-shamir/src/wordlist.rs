//! SLIP-39 Official 1024-word wordlist
//!
//! Source: https://github.com/satoshilabs/slips/blob/master/slip-0039/wordlist.txt

/// The official SLIP-39 wordlist (1024 words, 10 bits per word)
...
/// Get word index (0-1023) for a word
...
/// Get word at index (0-1023)
pub fn index_to_word(index: u16) -> Option<&'static str> {
    SLIP39_WORDLIST.get(index as usize).copied()
}
...
mod tests {
    ...
    fn test_wordlist_length() {
        assert_eq!(SLIP39_WORDLIST.len(), 1024);
    }

    #[test]
    fn test_first_last_words() {
        assert_eq!(SLIP39_WORDLIST[0], "academic");
        assert_eq!(SLIP39_WORDLIST[1023], "zero");
    }

    #[test]
    fn test_word_to_index() {
        assert_eq!(word_to_index("academic"), Some(0));
        assert_eq!(word_to_index("zero"), Some(1023));
        assert_eq!(word_to_index("satoshi"), Some(781)); // Famous word in the list!
        assert_eq!(word_to_index("notaword"), None);
    }

    #[test]
    fn test_index_to_word() {
        assert_eq!(index_to_word(0), Some("academic"));
        assert_eq!(index_to_word(1023), Some("zero"));
        assert_eq!(index_to_word(1024), None);
    }

    #[test]
    fn test_roundtrip() {
        for (i, word) in SLIP39_WORDLIST.iter().enumerate() {
            assert_eq!(word_to_index(word), Some(i as u16));
            assert_eq!(index_to_word(i as u16), Some(*word));
        }
    }
}
