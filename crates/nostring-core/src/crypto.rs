...
fn derive_key(password: &str, salt: &[u8; SALT_LEN]) -> Result<[u8; ARGON2_OUTPUT_LEN], CryptoError> {
...
pub fn encrypt_seed(seed: &[u8; 64], password: &str) -> Result<EncryptedSeed, CryptoError> {
    ...
    Ok(EncryptedSeed { salt, nonce, ciphertext })
}
...
pub fn decrypt_seed(encrypted: &EncryptedSeed, password: &str) -> Result<[u8; 64], CryptoError> {
    ...
    let plaintext = cipher
        ...
        .map_err(|_| CryptoError::DecryptionFailed("Invalid password or corrupted data".to_string()))?;
    
    // Verify length
    if plaintext.len() != 64 {
        return Err(CryptoError::DecryptionFailed("Invalid seed length".to_string()));
    }
